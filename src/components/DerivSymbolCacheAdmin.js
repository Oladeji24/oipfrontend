...
import { clearDerivSymbolCache, getDerivSymbolInfo } from '../api/deriv';

const DerivSymbolCacheAdmin = () => {
  ...
  const handleClearCache = async (e) => {
    ...
    setResult(null);
    try {
      const res = await clearDerivSymbolCache(symbol);
  ...
  const handleFetchInfo = async () => {
    ...
    setSymbolInfo(null);
    try {
      const res = await getDerivSymbolInfo(symbol);
  ...
  return (
    <div className="my-4 p-3 border rounded bg-light shadow-sm">
      <h5 className="mb-3">Deriv Symbol Cache & Info (Admin)</h5>
      <Form onSubmit={handleClearCache} className="d-flex align-items-center flex-wrap gap-2">
        <Form.Control
          type="text"
          placeholder="Enter symbol (e.g. frxUSDJPY)"
          value={symbol}
...
export default DerivSymbolCacheAdmin;
